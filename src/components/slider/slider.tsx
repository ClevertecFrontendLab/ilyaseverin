    ...
    <div className='swiperWrapper'>
      {windowWidth > 1100 && (
        ...
        >
          {item.images.map((image: any) => (
            <SwiperSlide data-test-id='slide-mini' key={image} role='presentation'>
              <img className='imageTop' src={image.url! ? host + image.url : AltImage} alt='{Image}' />
            </SwiperSlide>
          ))}
        </Swiper>
      )}
      {windowWidth > 1100 && length > 1 && (
        ...
        >
          {item.images.map((image: any) => (
            <SwiperSlide data-test-id='slide-mini' key={image} role='presentation'>
              <img className='imageBottom' src={image.url! ? host + image.url : AltImage} alt='{Image}' />
            </SwiperSlide>
          ))}
        </Swiper>
      )}
