...
export const CardComponent: React.FC = () => {
    ...
    return <div>
        ...
        {(!filtered.length && find !== '') &&
            <div data-test-id='search-result-not-found' className={styles.notFoundText}>По запросу ничего не найдено</div>
        }
        {(filtered.length === 0) && (find === '') &&
            <div data-test-id='empty-category' className={styles.notFoundText}>В этой категории книг ещё нет</div>
        }
        <div className={styles.boooksContentWrapper}>
            {filtered.map((item: any) => filter ? (
                <ItemCard
                    key={item.id} item={item} find={find} />
